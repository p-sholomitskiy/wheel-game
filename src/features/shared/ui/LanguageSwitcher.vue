<template>
  <Teleport to="body" :disabled="embedded">
    <div
      ...
      :class="{ 'language-switcher--embedded': embedded }"
...
const props = withDefaults(defineProps<{ position?: "desktop" | "mobile"; embedded?: boolean }>(), {
  position: "desktop",
  embedded: false,
});
...
const floatingPosition = computed(() =>
  props.position === "mobile"
    ? { top: "16px", right: "16px", left: "auto" as const }
    : { top: "40px", right: "120px", left: "auto" as const },
);

const containerStyle = computed(() =>
  props.embedded ? {} : { position: "fixed" as const, ...floatingPosition.value, zIndex: 60 },
);
...
const locales = computed<Locale[]>(() =>
  LOCALE_ORDER.map((code) => ({ code, label: texts.value.languageSwitcher.localeLabels[code] })),
);
const currentLocale = computed<Locale>(
  () => locales.value.find((l) => l.code === localeStore.current) ?? locales.value[0]!,
);
const otherLocales = computed(() =>
  locales.value
    .filter((l) => l.code !== currentLocale.value.code)
    .sort((a, b) => LOCALE_ORDER.indexOf(a.code) - LOCALE_ORDER.indexOf(b.code)),
);
...
function onDocPointerDown(e: PointerEvent) {
  ...
  if (!el.contains(e.target as Node)) isOpen.value = false;
}
