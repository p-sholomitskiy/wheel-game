  ...
  <div class="wheel-section">
        ...
        <div class="wheel-labels" aria-hidden="true">
          ...
          >
            <template v-if="label === `${CASINO_NAME}`"
              ><img class="wheel-label__logo" src="/ui/logo-casino.svg" alt="Casino Logo"
            /></template>
            <template v-else
              ><span
    ...
    <Teleport v-if="bonusTeleportEnabled" defer to="#bonus-reward-anchor">
      <div
        v-if="bonusEligible"
        class="bonus-message"
        :class="{ 'bonus-message--open': bonusRevealOpen }"
      >
...
const { canSpin, completedSpins, recordSpinComplete } = useWheelSpinStorage();
...
const firstPrizeSectorIndex = computed(() =>
  findSectorIndexForPrizeLabel(wheelLabels.value, texts.value.wheel.highlightedLabels[0]),
);
const secondPrizeSectorIndex = computed(() =>
  findSectorIndexForPrizeLabel(wheelLabels.value, texts.value.wheel.highlightedLabels[1]),
);
const { isSpinning, spinWheel, wheelStyle } = useWheelMotion({
  getTargetNormalizedAngleDeg() {
    if (!canSpin.value) return null;
    const labels = wheelLabels.value;
    const n = labels.length;
    if (n === 0) return null;
    const idx =
      completedSpins.value === 0 ? firstPrizeSectorIndex.value : secondPrizeSectorIndex.value;
    return sectorIndexToNormalizedWheelAngle(idx, n);
  },
  onSpinAnimationComplete: recordSpinComplete,
});
const bonusRevealOpen = ref(false);
const bonusEligible = computed(() => completedSpins.value >= 2);
function prefersReducedMotion() {
  if (typeof window === "undefined") return false;
  return window.matchMedia?.("(prefers-reduced-motion: reduce)").matches === true;
}
watch(
  [bonusEligible, bonusTeleportEnabled],
  async ([eligible, teleported]) => {
    if (!eligible || !teleported) {
      bonusRevealOpen.value = false;
      return;
    }
    if (prefersReducedMotion()) {
      bonusRevealOpen.value = true;
      return;
    }
    bonusRevealOpen.value = false;
    await nextTick();
...
function wheelSectorLabelMatches(candidate: string, sectorLabel: string) {
  return normalizeWheelLabel(candidate) === normalizeWheelLabel(sectorLabel);
}

/** Индекс сектора в `labels` для остановки колеса; совпадение по нормализованному тексту (пробелы/переносы не мешают). */
function findSectorIndexForPrizeLabel(labels: string[], prizeLabel: string | undefined) {
  if (prizeLabel == null || prizeLabel === "") return 0;
  const idx = labels.findIndex((l) => wheelSectorLabelMatches(prizeLabel, l));
  return idx >= 0 ? idx : 0;
}
</script>
...
.bonus-message {
  --bonus-slide-ease: cubic-bezier(0.22, 1, 0.36, 1);
  --bonus-overlap-half: clamp(56px, 18vw, 76px);
...
.bonus-message__inner {
  ...
  opacity: 0;
  transition:
    transform 0.58s var(--bonus-slide-ease),
    opacity 0.42s ease 0.06s;
  will-change: transform, opacity;
}
.bonus-message--open .bonus-message__inner {
